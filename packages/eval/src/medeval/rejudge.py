"""Recompute judge metrics on a saved report, using the contexts stored with it.

`rescore` handles the deterministic metrics offline. Judge metrics couldn't be recomputed
at all: the only fix for a throttled judge was a full pipeline re-run, 15 minutes of
generation to repair scores whose inputs were already sitting on disk. `contexts` has been
persisted for exactly this purpose; this is the consumer.

The failure it's built against: a rate-limited judge doesn't raise. RAGAS returns NaN per
row, NaN becomes None, and the aggregate averages whatever survived, which is how one
report shipped `answer_relevancy` computed from one case out of sixty. So this module:
...
# faithfulness is the one that catches an answer that's correct but ungrounded, which is
# the defining failure mode here.
...
def rejudge(
    ...
    *,
    ...
    if not rows:
        raise RuntimeError(
            "nothing to rejudge: no qa rows with stored contexts. Older reports do not "
            "persist contexts and can only be re-judged by re-running the target."
        )

    # Resume. A throttled judge run takes tens of minutes, and without this a timeout or
    # a Ctrl-C throws away every call already paid for.
