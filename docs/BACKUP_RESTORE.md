...
RANGE-partitioned by day, and a dump that restored the parent table but not its daily
...
Correct recovery for Redis is an *empty* Redis. Proven in the chaos drill: with Redis stopped, the
...
### 2. DATABASE_URL was never set, so the Postgres drill was vacuous 🔴

`DATABASE_URL` is absent from `.env` and `.env.example`, so the API ran with history
**disabled** — and the drill "proved" resilience by stopping a database the application
...
This is the identical bug class to the empty `REDIS_URL` found earlier — and I had fixed
...
Same root cause as the Redis finding: history is read once and written once per
...
**Follow-up correction.** The breaker was added but not re-measured, and
